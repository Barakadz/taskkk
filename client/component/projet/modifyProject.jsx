...
const ModifyProject = ({ id, tire_projet, descri, chefp, dadebut, dafin, dep, filia, par }) => {
    ...
    const initialValues = {
        ...
        chefProjet: chefp ? chefp.split('-').map(ch => ({ value: ch, label: ch })) : [],
        direction: dep ? dep.split('-').map(d => ({ value: d, label: d })) : [],
        filiale: filia ? filia.split('-').map(f => ({ value: f, label: f })) : [],
        participant: par ? par.split('-').map(p => ({ value: p, label: p })) : [],
